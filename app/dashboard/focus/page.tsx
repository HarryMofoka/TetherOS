        ...
        <div className="flex bg-card border border-border rounded-full p-1 mb-12 shadow-sm">
          {(Object.keys(MODE_TIMES) as Mode[]).map(m => (
            <button 
              ...
              className={`px-6 py-2 rounded-full text-sm font-semibold transition-colors ${mode === m ? "bg-foreground text-background shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
            >
              {m}
            </button>
          ))}
